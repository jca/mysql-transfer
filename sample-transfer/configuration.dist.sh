...
#
# Variables used in the mysql-transfer.sh can be initialized here

LIMITEDDATA_TABLES=()

# Example with direct limit clause
LIMITEDDATA_TABLES+=("created > NOW() - INTERVAL 1 WEEK -- tracking")
...
LIMITEDDATA_TABLES+=("id > {{ MIN_ORDER_ID }} -- sales_order")
LIMITEDDATA_TABLES+=("sales_order_id > {{ MIN_ORDER_ID }} -- sales_order_line")